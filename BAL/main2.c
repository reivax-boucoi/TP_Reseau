...
int main(int argc, char **argv){
	...
	//		printf("SOURCE : lg_mesg_emis=%d, port=%d, nb_d'envois=%d, dest=%s n°%d,(%d)[%s]\r\n",i+1,l,msg);
			...
			printf("Reception : Demande recuperation de ses lettres,(%d)[%s]\r\n",l,msg);
			int ind_msg=0;
			while(l!=0){
				l=read(sockListen,msg,MSG_LENGTH);
				if(l==-1){
					printf("Failed Read\r\n");
					exit(1);
				}				
